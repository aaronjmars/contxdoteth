...
const REGISTRY_ADDRESS =
  "0xa2bbe9b6a4ca01806b1cfac4174e4976ce2b0d70" as Address;
...
export async function GET(
  ...
  try {
    ...
    if (selector === "0x3b3b57de") {
      ...
      const key = decodedBytes
        .slice(keyStart, keyStart + keyLength)
        .toString("utf8");
      ...
      const padding =
        paddingLength === 32 ? Buffer.alloc(0) : Buffer.alloc(paddingLength);
  ...
  try {
    ...
    switch (method) {
      ...
      case "getText":
        if (!key) {
          return NextResponse.json(
            { error: "Key required for getText" },
            { status: 400 }
          );
        }
        result = await basePublicClient.readContract({
